...
const ProductDetails = () => {
  ...
  const { data, isLoading, isFetching} = useGetProductByIdQuery(id ?? skipToken)
  console.log("data", data)
  const product = data?.data
  const images = product?.images || []
  const [activeImg, setActiveImg] = useState<string>("")

  // set first image default
  useEffect(() => {
    if (images.length > 0) {
    ...
    <div className='flex flex-col '>
      <div className='grid grid-cols-1 lg:grid-cols-2 p-1'>
        <div className='flex justify-center'>
          <div className="flex flex-col gap-3 overflow-y-auto p-2 sm:h-[600px] premium-scrollbar">
            {images.map((img: any) => (
              <div
                key={img.id}
                onClick={() => setActiveImg(img.url)}
                className={`w-full h-[90px] cursor-pointer rounded-lg border-2
                  ${activeImg === img.url
                    ? "ring-2 ring-red-500 ring-offset-2 scale-105 shadow-md"
                    : "border-gray-300 hover:border-red-400 hover:shadow-sm"
                  }`}
              >
                <img
                  src={img.url}
          ...
          <div className=" bg-gray-100 rounded-xl overflow-hidden sm:h-[600px]">
            <img
              src={activeImg}
              alt="product"
